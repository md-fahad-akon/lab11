...
A dictionary is used to select the operator. Operator names are used as keys (for example "redact", "replace", "initial"), and the corresponding operator classes are stored as values. When an operator is needed, the factory looks it up by name in this dictionary.
